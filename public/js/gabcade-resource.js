...
class GabcadeResource {
  ...
  post (url, body, bodyContentType) {
    ...
    try {
      ...
      console.error('bbm-resource.post error', error);
      throw error;
    }
    return promise;
  }
  ...
  doDelete (transaction, resolve, reject) {
    ...
    var onReadyStateChange = this.onReadyStateChange.bind(this, transaction, resolve, reject);
  ...
  onReadyStateChange (transaction) {
    var response;
    // console.debug('onreadystatechange', transaction.request.readyState);
    if (transaction.request.readyState === XMLHttpRequest.DONE) {
      if (transaction.request.status === 200) {
        // console.debug('resource loaded', transaction.request);
        ...
        // console.error('dmp-resource: onReadyStateChange',
